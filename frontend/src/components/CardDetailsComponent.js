...
function CardDetailsComponent(props) {
    ...
    const handleUpdate = (e) => {
        ...
        if(edit){
            console.log('new info submitted',{cardName,cardDesc,cardList})
            toggleEdit(false);
    ...
    return (
        <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
                ...
                {edit ?
                    <select onChange={handleListUpdate}>
                        {allLists && allLists.map((boardList,index)=>{
                            if(boardList.idList === listInfo.idList){
                                return <option defaultValue key={index} value={boardList.idList}>{boardList.name}</option>
                            } else {
                                return <option key={index} value={boardList.idList}>{boardList.name}</option>
                            }
                        })}
                    </select>:
